class Company {
  ...
  toString() {
    returnr`${this.username} ${this.salary} ${this.position}`;
  }
  ...
  bestDepartment() {
    let sortedDepartments = [...this.departments].sort(
      // let sortedDepartments = Array.from(this.departments).sort(
    ...
    bestDepartment.sort((a, b) => b.salary - a.salary);
...
Average salary: ${this._getAverageSalary(bestDepartmentName).toFixed(2)}
    `;
    let workersString = bestDepartmentWorkers((x) => x.toString()).join('\n');
    return `${bestDepartmentString}\n${workersString}`;
  }

  _getAverageSalary(departmentName) {
    let departmentWorkers = this.department.get(departmentName);
    let averageSalary =
      departmentWorkers.reduce((acc, w) => acc + w.salary, 0) /
      departmentWorkers.length;
